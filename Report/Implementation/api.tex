...
For the entire highlevel logic we resolve it as as presented in \cref{fig:tosym}.
\begin{figure}[h!]
\begin{lstlisting}
...
$$\texttt{forAll :: MProvable m a => [String] -> a -> SymbolicT m SBool}$$
and because \texttt{m} must be an \texttt{ExtractIO}$$, which Reader and State does not implement\cite{}.
