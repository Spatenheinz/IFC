...
The most complex argument for equivalence between the code and the semantics is the ``while'' construct. \cref{fig:evalS} line 11-16, shows how we evaluate it. We first evaluate the loop condition and the invariant. Starting from the bottom up, we have a wildcard pattern, which will always be matched when the invariant evaluates to something false.
