...
The store is simply a map from VNames to Integers. We want the store to be a State as the store after one monadic action should be chained with the next monadic action. This ensures that all variables are in scope for the rest of the program and hereby entails mutability.
...
We first evaluate the invariant, to directly follow the semantical rules. Hence, when the invariant does not hold, we handle this case as abnormal termination, as per rule \textit{while-i-false} from \cref{fig:semantic}, similarly to how we would do for a standard assertion.
...
In \cref{sec:typesystem} we describe a potential extension to \textit{While}, which could help alleviate this problem.
