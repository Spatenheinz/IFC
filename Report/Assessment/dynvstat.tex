...
Lets take a closer look at the multiplication example program from \autoref{figure:mult}. 
...
Now Z3 will no longer be able to prove the correctness of the program. The generated formula looks as follows:
...
The two first conjuctions in line $4$ and line $5$-$9$ will be true.
%, given that $q_{2} = -3, res_{2} = 6, \$a = 0, r = 2 $.
In the last term in line $10$, 
%$$(q_{2} \leq 0 \land res_{2} = (\$a - q_{2}) * r) \Rightarrow res_{2} = \$a * r$$
the RHS of the implication, will be true because
$$(-3 \leq 0 \land 6 = 3 * 2)$$
but the LHS will not evaluate to \textit{true}, as 
$$6 \neq 0 * 2$$

But we know that the program does in fact behave as intended, so is it correct that the prover falsifies the formula?
Once again we can verify that our program acts correctly, by doing the same modification to the whyML program in \autoref{fig:whyml}. 
Trying to prove the program correctness will as expected give a falsifiable counter example.
By this, we have a good certainty that our implementation works correctly, and requires the necessary loop invariants.
% \\~\\
