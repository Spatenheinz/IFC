...
the LHS of the implication will evaluate to \textit{true} as
$(-3 \leq 0 \land 6 = 3 * 2)$
but the RHS will evaluate to \textit{false}, since
$6 \neq 0 * 2$

But we know that the program does in fact behave as intended, so is it correct that the prover falsifies the formula? We can verify that our program acts correctly, by doing the same modification to the whyML program in \cref{fig:why3}.
