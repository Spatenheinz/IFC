...
\begin{itemize}
  ...
  These are designed to cover all the cases of the small-step semantics, and the properties uses generators for generating suitable input variables and statements, i.e. the body of an $if$-statement is generated automatically, but the equivalence relation is defined manually.
	...
	When given an input program, it should always be true that if the \textit{VC generator} computes a provable verification condition, then the \textit{Evaluator} will evaluate all assertions in the program to \verb\true\. 
	To test this, we generate random programs and assert that if the \textit{VC generator} can verify the program, then the \textit{Evaluator} will evaluate to \verb\true\. Here we use generators to generate input for the test. However, it has come to our attention that the generated programs are not very useful, but we will come back to this in the assessment in \autoref{sec:evaluation}.
